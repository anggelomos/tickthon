...
def _is_task_a_weight_measurement(task: Task, weight_measurement_list_id: Optional[str]) -> bool:
    """Checks if a task is a weight measurement."""
    if weight_measurement_list_id is None:
        return False
    return task.project_id == weight_measurement_list_id
